...
def upgrade() -> None:
    ...
    op.add_column(
        "roo_rules", sa.Column("is_default", sa.Boolean, server_default="false")
    )

    # Composite lookup index: (agreement_id, hs_heading) — used on every origin check
    op.create_index(
        "ix_roo_rules_agreement_heading", "roo_rules", ["agreement_id", "hs_heading"]
    )
    op.create_index(
        "ix_roo_rules_agreement_chapter", "roo_rules", ["agreement_id", "hs_chapter"]
    )
    op.create_index(
        "ix_roo_rules_agreement_subheading",
        "roo_rules",
        ["agreement_id", "hs_subheading"],
    )
...
def downgrade() -> None:
    ...
    for col in [
        ...
        "ts_exceptions",
        "secondary_rule_type",
        "secondary_value_threshold",
        "secondary_rvc_method",
        "hs_range_from",
        "hs_range_to",
        "priority",
        "source_reference",

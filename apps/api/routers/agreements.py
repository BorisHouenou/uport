...
async def list_agreements(
    ...
    origin_country: str | None = Query(
        None, description="ISO-3166 alpha-2 country code"
    ),
...
async def classify_hs_code(
    ...
    description: str = Query(
        ..., description="Product description for AI classification"
    ),
):
    """
    ...
    task = classify_hs_code_task.delay(
        description=description, org_id=current_user["org_id"]
    )
    return HSCodeClassification(
        task_id=task.id, status="classifying", description=description
    )

...
def _build_pdf(
    cert_number: str, cert_type: str, org_name: str, shipment, determination, product
) -> bytes:
    ...
    title_style = ParagraphStyle(
        "T", parent=styles["Heading1"], fontSize=16, spaceAfter=4, alignment=1
    )
    ...
    label_style = ParagraphStyle(
        "L", parent=styles["Normal"], fontSize=8, textColor=colors.HexColor("#777777")
    )
    value_style = ParagraphStyle(
        "V", parent=styles["Normal"], fontSize=10, spaceAfter=4
    )
...
async def generate_certificate(
    ...
    ship_result = await db.execute(
        select(Shipment).where(
            Shipment.id == payload.shipment_id, Shipment.org_id == org_id
        )
    )
    ...
    det_result = await db.execute(
        select(OriginDetermination).where(
            OriginDetermination.id == payload.determination_id
        )
    )
    ...
    pdf_bytes = _build_pdf(
        cert_number, cert_type, org_name, shipment, determination, product
    )
...
async def download_certificate(
    ...
    if not pdf_bytes:
        raise HTTPException(
            status_code=404,
            detail="Certificate file not found on server — regenerate it",
        )
